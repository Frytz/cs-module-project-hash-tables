...
NOTES:
    What are hashTables?
        Hash tables are a data structure that stores data in an associative manner.
        The data is stored in an array manner.
        access is fast if we know the index of the desire data.
        This makes insert and get functions very fast regardless of the size of the data structure
        The hash table uses an array as a storage medium while using hash techniques to generate indexs where the element is to be stored or found.

    Hashing:
         Hashing is a technique used to convert a range of key values into a range of indexes of an array. 
         One method is using the modulo (%) operator to get a range of key values: ex. 102 #value of 102 % 8 #modded by 8 = 6 #returns a remainder of 6 that will be used for our index
    
    Linear Probing:
        A method used when hash collisions occur.
        If a hash function retunrn n index that is already in use, it will search for the next available empty index to place it in
        Example:
            1: 2 % 20 = 2
            2: 42 % 20 = 2

            #1 will be the first to be placed in index 2 and since #2 also return a result of 2 and since 2 is already taken, it will be placed in index 3 if 3 is available
